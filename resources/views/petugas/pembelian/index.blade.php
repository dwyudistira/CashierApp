<x-app-layout>
                    ...
                    <div class="card shadow-sm border-0 p-3 text-center">
                        ...
                        <div class="card-body">
                            ...
                            <div class="d-flex align-items-center justify-content-center">
                                <button class="btn btn-sm me-2" onclick="kurangi({{ $product->id }}, {{ $product->price }}, '{{ $product->name }}')">-</button>
                                <span id="jumlah-{{ $product->id }}" class="fw-bold">0</span>
                                <button class="btn btn-sm ms-2" onclick="tambah({{ $product->id }}, {{ $product->price }}, '{{ $product->name }}')">+</button>
                            </div>
    ...
    <div class="text-center mt-4">
        <a href="{{ route('petugas.pembelian.detail') }}" onclick="kirimDataKeHalamanBerikutnya()" class="btn btn-primary">Selanjutnya</a>
    </div>

    <script>
        let cart = {}; // Menyimpan produk yang dipilih
        ...
        function kurangi(id, harga, nama) {
            ...
            if (jumlah === 0) {
                delete cart[id]; // Hapus jika jumlah = 0
            } else {
                cart[id] = { id, nama, jumlah, subtotal: jumlah * harga };
            }
        ...
        function kirimDataKeHalamanBerikutnya() {
            localStorage.setItem("cart", JSON.stringify(cart)); // Pastikan tersimpan
            window.location.href = "{{ route('petugas.pembelian.detail') }}"; // Arahkan ke halaman berikutnya
        }
